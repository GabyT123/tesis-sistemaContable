...
// Inicio de la app
const Solicitude151al300 = (props: Props) => {
  const { auth } = useAuth();
  ...
  const loadData = async () => {
    ...
    if (
      ...
      var response = await HttpClient(
        "/api/solicitude/solicitudeHistory150-300",
        "GET",
        auth.userName,
        auth.role
      );
      ...
      var response = await HttpClient(
        "/api/solicitude/solicitudeHistory150-300?dates=" +
          props.dates[0] +
          "¡" +
          props.dates[1],
        "GET",
...
export default Solicitude151al300;
